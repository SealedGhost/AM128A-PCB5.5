          ...
          while(pBuly){
             if(pBuly->pBoatLink->Boat.user_id != refMMSI){
                if(pBuly->pBoatLink->mntState == MNTState_Triggered){
                   if(    pBuly->pBoatLink->Boat.longitude  
                       && pBuly->pBoatLink->Boat.longitude < refLg  
                       && refLg -pBuly->pBoatLink->Boat.longitude < minDiff )
                   {
                      minDiff  = refLg -pBuly->pBoatLink->Boat.longitude;
          ...
          while(pBuly){
             if(pBuly->pBoatLink->Boat.user_id != refMMSI){
                if(pBuly->pBoatLink->mntState == MNTState_Triggered){
                   if(    pBuly->pBoatLink->Boat.longitude  
                       && pBuly->pBoatLink->Boat.longitude > refLg  
                       && pBuly->pBoatLink->Boat.longitude -refLg < minDiff )
                   {
                      minDiff  = pBuly->pBoatLink->Boat.longitude -refLg;
                      potentialSnapLink  = pBuly->pBoatLink;
                      potentialSnapType  = 1;
                   }
                }
     ...
     else if(Dir_y == 1)
     {
       ...
       if(BULY_getValidNumber() > 0)
       {
          ...
          while(pBuly){
             if(pBuly->pBoatLink->Boat.user_id != refMMSI){
                if(pBuly->pBoatLink->mntState == MNTState_Triggered){
                   if(    pBuly->pBoatLink->Boat.latitude  
                       && pBuly->pBoatLink->Boat.latitude < refLt 
                       && refLt -pBuly->pBoatLink->Boat.latitude < minDiff )
                   {
                      minDiff  = refLt -pBuly->pBoatLink->Boat.latitude;
                      potentialSnapLink  = pBuly->pBoatLink;
                      potentialSnapType  = 1;
                   }
                }
        ...
        for(i=0; i<N_boat; i++)
        {
           if(SimpBerthes[i].pBerth->isInvader)
           {
              if(SimpBerthes[i].pBerth->Boat.user_id != refMMSI)
              {
                 ...
                 {
                    minDiff  = SimpBerthes[i].latitude - refLt;
                    potentialSnapLink  = SimpBerthes[i].pBerth;
                    potentialSnapType  = 1;
                 }
              }
          ...
          while(pBuly){
             if(pBuly->pBoatLink->Boat.user_id != refMMSI){
                if(pBuly->pBoatLink->mntState == MNTState_Triggered){
                   if(    pBuly->pBoatLink->Boat.latitude  
                       && pBuly->pBoatLink->Boat.latitude > refLt 
                       && pBuly->pBoatLink->Boat.latitude -refLt< minDiff )
                   {
                      minDiff  = pBuly->pBoatLink->Boat.latitude -refLt;
                      potentialSnapLink  = pBuly->pBoatLink;
                      potentialSnapType  = 1;
                   }
                }
